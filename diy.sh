...
# 应用 BeeconMini SEED AC2 补丁
echo "Applying BeeconMini SEED AC2 patches..."
if [ -f "ac2/patch/apply-patches-safe.sh" ]; then
    chmod +x ac2/patch/apply-patches-safe.sh
    ./ac2/patch/apply-patches-safe.sh
else
    echo "Warning: AC2 patch script not found, skipping device-specific patches"
fi